...
## 2026-04-21 - [Optimize HTTP Response Writes in Hot Paths]
**Learning:** Calling `w.Write([]byte("string"))` on an `http.ResponseWriter` forces a string-to-byte-slice cast. Because the slice escapes to the `io.Writer` interface, the Go compiler allocates the byte slice on the heap, increasing garbage collection pressure, especially on high-frequency endpoints like `/healthz`.
**Action:** Use `io.WriteString(w, "string")` instead. The standard library's HTTP implementation implements `io.StringWriter`, which allows it to write strings directly without allocating a new byte slice on the heap. This simple refactoring significantly reduces GC pressure for static responses.
