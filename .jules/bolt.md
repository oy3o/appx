...
## 2026-04-27 - [Optimize String Writing in HTTP Handlers]
**Learning:** In Go HTTP handlers, avoiding heap allocations from `[]byte("string")` casts when writing static strings to `http.ResponseWriter` is important for performance. Using `io.WriteString(w, "string")` leverages the underlying `io.StringWriter` interface to write strings without escaping to the heap.
**Action:** Use `io.WriteString(w, "string")` instead of `w.Write([]byte("string"))` in HTTP handlers to prevent unnecessary memory allocations, especially in high-frequency endpoints like `/healthz`.
