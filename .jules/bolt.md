...
## 2026-04-19 - [Avoid Heap Allocations in HTTP Response Writes]
**Learning:** In Go HTTP handlers, avoiding heap allocations from `[]byte("string")` casts when writing static strings to `http.ResponseWriter` is important for high-frequency endpoints. `w.Write([]byte("string"))` causes unnecessary allocations because it creates a new byte slice.
**Action:** Use `io.WriteString(w, "string")` instead of `w.Write([]byte("string"))`. It leverages the underlying `io.StringWriter` interface (implemented by `http.ResponseWriter` types) to write strings without escaping to the heap.
