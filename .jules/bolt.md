...
## 2026-04-13 - [Optimize static string writes in HTTP handlers]
**Learning:** Writing static strings to an `http.ResponseWriter` using `w.Write([]byte("string"))` causes an unnecessary heap allocation because it casts the string to a byte slice which escapes. `http.ResponseWriter` implements `io.StringWriter`, so we can use `io.WriteString(w, "string")` to write strings directly without allocating memory.
**Action:** When writing static strings in HTTP handlers (especially high-frequency ones like `/healthz`), always use `io.WriteString(w, "string")` instead of `w.Write([]byte("string"))` to reduce garbage collection overhead.
